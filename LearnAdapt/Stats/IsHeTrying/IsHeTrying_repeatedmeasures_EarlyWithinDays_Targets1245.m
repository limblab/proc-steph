function pvals=IsHeTrying_repeatedmeasures_EarlyWithinDays_Targets1245(IsHeTryingAllDaysPerTargetCell_Normal,IsHeTryingAllDaysPerTargetCell_Reflected,Day)
...
% Rows by columns: Normal/Reflected are rows and columns are targets
    ...
    for Tgt=1:8
        NormalData(1:NumOfTrials,Tgt) =  IsHeTryingAllDaysPerTargetCell_Normal{Day,1}{1,Tgt}(1:NumOfTrials);
        ReflectedData(1:NumOfTrials,Tgt) = IsHeTryingAllDaysPerTargetCell_Reflected{Day,1}{1,Tgt}(1:NumOfTrials);
    end
...
% Make a table
t = table(Labels,AllData(:,1),AllData(:,2),AllData(:,4),AllData(:,5),...
'VariableNames',{'TaskType','target1','target2','target4','target5'});
Meas = dataset([1 2 3 4]','VarNames',{'Measurements'});  


% Fit a repeated measures model, where the measurements are the responses
% and the TaskType is the predictor variable. 
rm = fitrm(t,'target1-target5~TaskType','WithinDesign',Meas); 
