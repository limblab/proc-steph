...
FileName = {'LongPert_Jango_073115_ref', 'LongPert_Jango_080415_rot',...
    ...
    'LongPert_Jango_090615_ref'};
    
% no longer including 'LongPert_Jango_092515_ref',...
...
% 'LongPert_Kevin_05042015_ref'};

% no longer including:'LongPert_Kevin_06132015_rot'...'LongPert_Kevin_06142015_ref','LongPert_Kevin_06252015_ref'};
...
[BaselineSuccessPoints{i}, firstBinarySuccess_baseline, lastBinarySuccess_baseline] = ComputeTotalSuccessOverTime_trialbasis(TrialsStructBaseline, TrialsPerEpoch);
% BASELINE STATS
% calculate significance of the slope for the baseline points. Test: is slope significantly different than 0?
 Baseline_pValue{i} = getCHIpValue(BaselineSuccessPoints{i},TrialsPerEpoch);
...
%    NormalizedReflectedSuccessPoints=ReflectedSuccessPoints{i}./BaselineSuccessPoints{i};
    % REFLECTED STATS
    Reflected_pValue{i} = getCHIpValue(ReflectedSuccessPoints{i},TrialsPerEpoch);
        ...
        % Normalized success
        %NormalizedRotatedSuccessPoints=RotatedSuccessPoints{i}./BaselineSuccessPoints{i};
        % ROTATED STATS
...
NormalizedRotatedSuccessPoints NormalizedReflectedSuccessPoints
...
%             plot( (j-.1):.1:(j-.1)+.1*length(ReflectedSuccessPoints{j})-.1 , ReflectedSuccessPoints{j},'r.-','MarkerSize',15,'LineWidth',LineWidth)
%         else
...
%     end
    if ~isempty(Rotated_pValue{j})
        if (Rotated_pValue{j})<=.05
            plot( (j-.1):.1:(j-.1)+.1*length(RotatedSuccessPoints{j})-.1 , RotatedSuccessPoints{j},'g.-','MarkerSize',15,'LineWidth',LineWidth)
        else
