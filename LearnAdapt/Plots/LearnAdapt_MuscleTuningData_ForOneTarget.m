function [EMGmean, EMGste] = LearnAdapt_MuscleTuningData_ForOneTarget(binnedData,epochLength,TargetNum)
...
for numEMG = 1:length(binnedData.emgdatabin(1,:))
    for NumTargets = 1:length(unique(binnedData.trialtable(:,10)))
        TargetTrialIndices = find(binnedData.trialtable(:,10)==TargetNum);
