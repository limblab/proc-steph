function Vout = interpRewritten(varargin)
...
%
%   PP = INTERP1(X,V,METHOD,'pp') is not recommended. Use griddedInterpolant
%   instead.
...
if ndataarg == 2
    [V, orig_size_v] = parseValuesV(varargin{1});
    Xq = varargin{2};
    X =(1:size(V,1))';
elseif ndataarg == 3
    X = varargin{1};
    ...
    [V, orig_size_v] = parseValuesV(varargin{2});
    X = X(:);
    if any(diff(X)<0)
        [X, idx] = sort(X);
        V = V(idx,:);
...
%-------------------------------------------------------------------------%
% Parses the values V at the the grid points X.
function [V, orig_size_v] = parseValuesV(Vin)
if ~isfloat(Vin)
    error(message('MATLAB:interp1:NonFloatValues'));
end
if isvector(Vin)
    orig_size_v = size(Vin);
    V = Vin(:); % Reorient not considered a resize
else
    orig_size_v = size(Vin);
    n = orig_size_v(1);
    ds = orig_size_v(2:end);
    prodDs = prod(ds);
    V = reshape(Vin,[n prodDs]);
end
end