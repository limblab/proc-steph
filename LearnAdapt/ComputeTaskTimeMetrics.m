...
for N = 1:length(Alltrialtable(:,1))
    timediff = abs(out_struct.pos(:,1) - Alltrialtable(N,7));
    GoCueIndex(N,1) = max(find(timediff == min(timediff)));
    timediff = abs(out_struct.pos(:,1) - Alltrialtable(N,8));
    EndTrialIndex(N,1) = max(find(timediff == min(timediff)));
end

for i = 1:length(Alltrialtable(:,1)) % Loop through all trials
    
    if Alltrialtable(i,9)~=82
        DialInTime(i,1) = -1;
        T2Tfirst(i,1) = -1;
        T2Tfirst(i,2) = out_struct.pos(GoCueIndex(i));
        T2Tlast(i,1) = -1;
        T2Tlast(i,2) = out_struct.pos(GoCueIndex(i));
        NumTargetEntries(i,1)=0;
    else
        
        
        % Get target side size
        sideX = Alltrialtable(i,4)-Alltrialtable(i,2);
        sideY = Alltrialtable(i,3)-Alltrialtable(i,5);
        % Get lower left target coordinates
        LLx = Alltrialtable(i,2); LLy = Alltrialtable(i,5);
...
NumOfTargets = max(Alltrialtable(:,10));
for N=1:NumOfTargets
    T2TfirstStruct.(['Target' num2str(N)]) = T2Tfirst(find(Alltrialtable(:,10) == N),:);
    T2TlastStruct.(['Target' num2str(N)]) = T2Tlast(find(Alltrialtable(:,10) == N),:);
    DialInTimeStruct.(['Target' num2str(N)]) = DialInTime(find(Alltrialtable(:,10) == N),:);
    TargetEntriesStruct.(['Target' num2str(N)]) = NumTargetEntries(find(Alltrialtable(:,10) == N),:);
...
T2TfirstStruct.T2TfirstFull(:,1) = T2Tfirst(:,1); T2TfirstStruct.T2TfirstFull(:,2) = T2Tfirst(:,2); T2TfirstStruct.T2TfirstFull(:,3) = Alltrialtable(:,10); T2TfirstStruct.T2TfirstFull(:,4) = Alltrialtable(:,9);
T2TfirstStruct.T2TfirstSummary(NumOfTargets+1,1) = mean(T2TfirstStruct.T2TfirstFull(:,1));
%--------------------------------------------------------------------------------------------
T2TlastStruct.T2TlastFull(:,1) = T2Tlast(:,1); T2TlastStruct.T2TlastFull(:,2) = T2Tlast(:,2); T2TlastStruct.T2TlastFull(:,3) = Alltrialtable(:,10);T2TlastStruct.T2TlastFull(:,4) = Alltrialtable(:,9);
T2TlastStruct.T2TlastSummary(NumOfTargets+1,1) = mean(T2TlastStruct.T2TlastFull(:,1));
%--------------------------------------------------------------------------------------------
DialInTimeStruct.DialInTimeFull(:,1) = DialInTime; DialInTimeStruct.DialInTimeFull(:,2) = Alltrialtable(:,10);DialInTimeStruct.DialInTimeFull(:,3) = Alltrialtable(:,9);
DialInTimeStruct.DialInTimeSummary(NumOfTargets+1,1) = mean(DialInTimeStruct.DialInTimeFull(:,1));
%--------------------------------------------------------------------------------------------
TargetEntriesStruct.TargetEntriesFull(:,1) = NumTargetEntries; TargetEntriesStruct.TargetEntriesFull(:,2) = Alltrialtable(:,10);TargetEntriesStruct.TargetEntriesFull(:,3) = Alltrialtable(:,9);
