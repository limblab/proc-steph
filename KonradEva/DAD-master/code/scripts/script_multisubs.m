%function [Xte, Yte, Tte, Xtr, Ytr, Ttr] = script_multisubs(binnedData)
...
%whichtarg = [1, 4, 5, 8]; % which targets to select reaches from
M1{1} = 'FA'; 
...
%% get data
[Y1,T1,X1] = compile_neuraldata(whichtarg,numSamp,numDelay, binnedData);
...
subplot(2,2,3), colorData(Res{numIter}.V(:,1:3),Tte),
