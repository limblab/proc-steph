...
if SpringFile==1
    ...
    [SonI_R2, ~, SonIpred, SonI_vaf, SonI_mse, SonIactTrunk SonI_indivVAF]=OneDecoderOnManyFolds(SprModel,IsoTest,foldlength);
    [SonW_R2, ~, SonWpred, SonW_vaf, SonW_mse, SonWactTrunk SonW_indivVAF]=OneDecoderOnManyFolds(SprModel,WmTest,foldlength);
...
% Make VAF struct with multivariate VAF data
...
if SpringFile==0
    ...
    VAFstruct.SonI_vaf_mean = [];
    VAFstruct.SonW_vaf_mean = [];
    VAFstruct.SonS_vaf_ste = [];
    VAFstruct.HonS_vaf_ste = [];
    VAFstruct.IonS_vaf_ste = [];
    VAFstruct.WonS_vaf_ste = [];
    VAFstruct.SonI_vaf_ste = [];
    VAFstruct.SonW_vaf_ste = [];
    ...
    VAFstruct.SonI_vaf_mean = mean(SonI_vaf);
    VAFstruct.SonW_vaf_mean = mean(SonW_vaf);
    VAFstruct.SonI_vaf_ste = std(SonI_vaf)/sqrt(length(SonI_vaf));
    VAFstruct.SonW_vaf_ste = std(SonW_vaf)/sqrt(length(SonW_vaf));
...
% Make VAF struct for individual muscle preds (this should be for taking
% the mean)
