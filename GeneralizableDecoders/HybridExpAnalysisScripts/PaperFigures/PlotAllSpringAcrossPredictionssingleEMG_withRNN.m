function PlotAllSpringAcrossPredictionssingleEMG_withRNN(SprBinned,SonSact,SonSpred,IonSpred,WonSpred,HonSpred,RNNHonSpred,emgInd,save,foldername, filename)
...
plot(x,RNNHonSpred(:,emgInd),'y','LineWidth', linewidth)
xlabel('Time (sec)')
...
muscleVAF.RNNHonS = calculateVAF(RNNHonSpred(startbinned:stopbinned,emgInd),SonSact(startbinned:stopbinned,emgInd));

legend('Actual',strcat('Within | vaf=',num2str(muscleVAF.SonS)),strcat('Hybrid | vaf=',num2str(muscleVAF.HonS)),strcat('Iso | vaf=',num2str(muscleVAF.IonS)),strcat('Move | vaf=',num2str(muscleVAF.WonS)),strcat('RNN | vaf=',num2str(muscleVAF.RNNHonS)))
MillerFigure
