function PlotAllMovementAcrossPredictionssingleEMG_withRNN(WmBinned,WonWact,WonWpred,IonWpred,SonWpred,HonWpred,RNNHonWpred,emgInd,save,foldername, filename)
...
plot(x,RNNHonWpred(:,emgInd),'y','LineWidth', linewidth)
xlabel('Time (sec)')
...
muscleVAF.RNNHonW = calculateVAF(RNNHonWpred(startbinned:stopbinned,emgInd),WonWact(startbinned:stopbinned,emgInd));


legend('Actual',strcat('Within | vaf=',num2str(muscleVAF.WonW)),strcat('Hybrid | vaf=',num2str(muscleVAF.HonW)),strcat('Iso | vaf=',num2str(muscleVAF.IonW)),strcat('Spring | vaf=',num2str(muscleVAF.SonW)),strcat('RNN | vaf=',num2str(muscleVAF.RNNHonW)))
MillerFigure
